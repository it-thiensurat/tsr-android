...
public class AddCutDivisorContractInputInfo extends BHParcelable {
...
//    public String LastAssignTaskType;      // ???? TaskType ???????????????????? (SaleAudit=????????????????????????????????????? / SalePaymentPeriod=??????????????????????????????????????????????)
    ...
    public static AddCutDivisorContractInputInfo from(CutDivisorContractInfo coc) {
        AddCutDivisorContractInputInfo info = new AddCutDivisorContractInputInfo();
        info.CutDivisorContractID = coc.CutDivisorContractID;
        ...
        info.CutDivisorContractPaperID = coc.CutDivisorContractPaperID;
