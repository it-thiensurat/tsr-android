...
public class SaleUnfinishedFragment_preorder extends BHFragment {
	...
	private static final int UNFINISHED = R.array.sale_unfinished_menu_preorder;
	int intStatuscode;
			...
			private void selectedMenu(int position, int titleID) {
				...
				switch (titleID) {
					...
					case R.string.sale_unfinished_employee:
						if (intStatuscode <= 6) {
							if (intStatuscode >= 2) {
								showNextView(BHFragment.newInstance(SaleScanEmployeesFragment_preorder.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
...
//							showNextView(BHFragment.newInstance(SaleCustomerAddressMainFragment.class));	// PARADA [Tab-Address]
								showNextView(BHFragment.newInstance(New2SaleCustomerAddressCardFragment_preorder.class));    // PARADA [Separate-Address]
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
