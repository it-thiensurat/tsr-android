...
import th.co.thiensurat.fragments.sales.preorder.SaleContractPrintFragment_preorder;
...
import th.co.thiensurat.fragments.sales.preorder.SaleMoreDetailAddress_preorder;
import th.co.thiensurat.fragments.sales.preorder.SalePayFragment_preorder;
...
public class SaleUnfinishedFragment_sale_Q extends BHFragment {
	...
	public String getProductID,getProductName,getProductSerialNumber,getCONTNO,getRefNo;

	private Data data;

	@InjectView
	private ListView lvMenu;
	private static final int UNFINISHED = R.array.sale_unfinished_menu_sale_Q;
	int intStatuscode;
				...
				switch (titleID) {
					case R.string.sale_unfinished_employee:
						if (intStatuscode <= 6) {
							...
							//	BarcodeScan();
								showNextView(BHFragment.newInstance(SaleScanEmployeesFragment_sale_Q.class));
					...
					case R.string.sale_unfinished_customer:
						if (intStatuscode <= 6) {
							if (intStatuscode >= 2) {
								showNextView(BHFragment.newInstance(New2SaleCustomerAddressCardFragment_sale_Q.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
						...
						break;
					case R.string.sale_unfinished_scan_product_preorder:

						if (intStatuscode <= 6) {
							if (intStatuscode >= 3) {
//							showNextView(BHFragment.newInstance(SaleCustomerAddressMainFragment.class));	// PARADA [Tab-Address]
								showNextView(BHFragment.newInstance(Product_sale_Q.class));    // PARADA [Separate-Address]
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_contract:
						...
						if (intStatuscode <= 8) {
							if (intStatuscode >= 7) {
								showNextView(BHFragment.newInstance(SaleContractPrintFragment_preorder.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_payment:
						...
						if (intStatuscode <= 8) {
							if (intStatuscode >= 8) {
								showNextView(BHFragment.newInstance(SaleFirstPaymentChoiceFragment_preorder.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
	...
	private void BarcodeScan() {
		...
		PreorderFragment fm = BHFragment.newInstance(PreorderFragment.class, new ScanCallBack() {
			...
			public void onResult(final BHParcelable data1) {
				// TODO Auto-generated method stub
				final PreorderFragment.Result barcodeResult = (PreorderFragment.Result) data1;
				(new BackgroundProcess(activity) {
					...
					protected void after() {
						...
						BarcodeScanSuccess(productInfo, barcodeResult.getProductID);
