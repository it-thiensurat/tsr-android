...
public class SaleUnfinishedFragment_preorder_setting extends BHFragment {
			...
			private void selectedMenu(int position, int titleID) {
				...
				switch (titleID) {
					...
					case R.string.sale_unfinished_employee:
						if (intStatuscode <= 6) {
							if (intStatuscode >= 2) {
								showNextView(BHFragment.newInstance(SaleScanEmployeesFragment_preorder_setting.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
...
//							showNextView(BHFragment.newInstance(SaleCustomerAddressMainFragment.class));	// PARADA [Tab-Address]
								showNextView(BHFragment.newInstance(New2SaleCustomerAddressCardFragment_preorder_setting.class));    // PARADA [Separate-Address]
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_package:
						...
						if (intStatuscode <= 6) {
							if (intStatuscode >= 4) {
								showNextView(BHFragment.newInstance(SaleListPackagePaymentFragment_setting.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_discount:
						...
						if (intStatuscode <= 6) {
							if (intStatuscode >= 5) {
								showNextView(BHFragment.newInstance(SalePayFragment_setting.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_detail_contract:
						...
						if (intStatuscode <= 6) {
							if (intStatuscode >= 6) {
								showNextView(BHFragment.newInstance(SaleDetailCheckFragment_setting.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
					...
					case R.string.sale_unfinished_contract:
						...
						if (intStatuscode <= 8) {
							if (intStatuscode >= 7) {
								showNextView(BHFragment.newInstance(SaleContractPrintFragment_setting.class));
							} else {
								showMessage("สถานะการทำงานของท่านยังมาไม่ถึง");
							}
						} else {
	...
	private void BarcodeScan() {
		...
		BarcodeScanFragment_preorder_setting fm = BHFragment.newInstance(BarcodeScanFragment_preorder_setting.class, new ScanCallBack() {
			...
			public void onResult(final BHParcelable data1) {
				// TODO Auto-generated method stub
				final BarcodeScanFragment_preorder_setting.Result barcodeResult = (BarcodeScanFragment_preorder_setting.Result) data1;
				(new BackgroundProcess(activity) {
